...
// Validasi parameter 'faktur'
if (!isset($_GET['faktur']) || empty($_GET['faktur'])) {
    die('Error: Parameter "faktur" tidak ditemukan.');
}

// Ambil faktur dari URL
$faktur = $_GET['faktur'];

// Query untuk mengambil detail transaksi berdasarkan faktur
$query = "SELECT 
            t.faktur AS invoice, 
            t.jumlah AS weight, 
            t.biaya AS total,
...
if (!$transaction) {
    die('Error: Transaksi dengan faktur "' . htmlspecialchars($faktur) . '" tidak ditemukan.');
}
